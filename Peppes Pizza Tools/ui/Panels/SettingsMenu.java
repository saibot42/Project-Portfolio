...
public class SettingsMenu extends JDialog implements ThemedComponent {
    ...
    public SettingsMenu(Window owner) {
        ...
        themeLabel = createSectionLabel("Theme");
        root.add(themeLabel);
        ...
        langLabel = createSectionLabel("Language");
        root.add(langLabel);
        root.add(Box.createVerticalStrut(12));
        ...
        closeBtn = createTextButton("Close");
    ...
    public void applyTheme() {
        Theme t = dashboardGUI.theme;
        title.setForeground(t.primaryTextColor());
        themeLabel.setForeground(t.primaryTextColor());
        langLabel.setForeground(t.primaryTextColor());
        closeBtn.setForeground(t.primaryTextColor());
        repaint();
    }
    ...
    private JPanel createThemeButtons() {
        ...
        row.setOpaque(false);
        row.add(createStyledButton(null, g2 -> {
            int cx = 60, cy = 22;
        ...
        row.add(createStyledButton(null, g2 -> {
            int cx = 60, cy = 22; int r = 6;
            g2.setColor(dashboardGUI.theme.primaryTextColor());
            g2.setStroke(new BasicStroke(2f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
            g2.drawOval(cx - r, cy - r, r * 2, r * 2);
            for (int i = 0; i < 8; i++) {
                double angle = Math.toRadians(i * 45);
                int x1 = (int)(cx + (r + 3) * Math.cos(angle)); int y1 = (int)(cy + (r + 3) * Math.sin(angle));
                int x2 = (int)(cx + (r + 7) * Math.cos(angle)); int y2 = (int)(cy + (r + 7) * Math.sin(angle));
    ...
    private JPanel createLanguageButtons() {
        JPanel row = new JPanel(new FlowLayout(FlowLayout.CENTER, 12, 0));
        row.setOpaque(false);
        row.add(createStyledButton("EN", null, e -> System.out.println("EN")));
        row.add(createStyledButton("NO", null, e -> System.out.println("NO")));
        return row;
    }

    private JButton createStyledButton(String text, IconPainter iconPainter, java.awt.event.ActionListener action) {
